/**
 * This file has all Firebase methods.
 */
...
var firebaseConfig = {
  ...
  initFirebaseAuth: function () {
    ...
    firebase.auth().onAuthStateChanged((user) => {
        if (user) {
                // signed in!
                // Now add a new document uid in collection "users"
                addData("users", user.id, {userName: user.displayName})
                return true;
        }
        else{ false; } 
    });
...
//   }
  ...
  getData: function (collection, document, callback ) {
      var db = firebase.firestore();
      var docRef = db.collection(collection).doc(document);
      docRef.get().then(function(doc) {
        if (doc.exists) {
            console.log("Document data:", doc.data()["rooms"]);
            callback(doc.data());            
        } else {
  ...
  addData: function (collection, document, data) {
    var db = firebase.firestore();
      if(document == null){
  ...
  saveMessage: function (messageText) {
